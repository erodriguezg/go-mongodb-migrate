...
//go:embed 1_global_migrate_test.go
var globalMirateTest string
...
func TestMigrationsRegistration(t *testing.T) {
	...
	err := Register(&globalMirateTest, func(db *mongo.Database) error {
		return nil
...
func TestMigrationMustRegistration(t *testing.T) {
	...
	globalMigrate = NewMigrate(nil)
	MustRegister(&globalMirateTest, func(db *mongo.Database) error {
		return nil
	}, func(db *mongo.Database) error {
		return nil
	})

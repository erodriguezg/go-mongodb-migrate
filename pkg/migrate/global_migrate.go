...
func internalRegister(migrationSource *string, up, down MigrationFunc, skip int) error {
	...
	hash, err := calculateHash(migrationSource)
	if err != nil {
		return err
	}
...
//	 )
//	 //go:embed migration.go
//	 var sourceCode string
//
//	 func init() {
//		 migrate.Register(&sourceCode, func(db *mongo.Database) error {
...
//		 })
//	 }
func Register(migrationSource *string, up, down MigrationFunc) error {
	return internalRegister(migrationSource, up, down, 2)
}

// MustRegister acts like Register but panics on errors.
func MustRegister(migrationSource *string, up, down MigrationFunc) {
	if err := internalRegister(migrationSource, up, down, 2); err != nil {
		panic(err)
	}
}
...
func SetMigrationsAutoRepair(enabled bool) {
	globalMigrate.SetMigrationsAutoRepair(enabled)
}

// SetMigrationsEnabled changes default behavior for enabled the process of migrations
func SetMigrationsEnabled(enabled bool) {
	globalMigrate.SetMigrationsEnabled(enabled)
}
